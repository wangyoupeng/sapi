...
let methods = {
  ...
  ShoppingCartData: async data => {
    ...
    for (let i = 0; i < data.length; i++) {
      const temp = data[i];
      const product = await productModel.GetProductById(temp.product_id);
...
module.exports = {
  ...
  GetShoppingCart: async ctx => {
    ...
    // 获取购物车信息
    const shoppingCart = await shoppingCartModel.GetShoppingCart(user_id);
    // 生成购物车详细信息
  ...
  AddShoppingCart: async ctx => {
    ...
    let tempShoppingCart = await shoppingCartModel.FindShoppingCart(user_id, product_id);
    //判断该用户的购物车是否存在该商品
    if (tempShoppingCart.length > 0) {
      //如果存在则把数量+1
      const tempNum = tempShoppingCart[0].num + 1;

      const product = await productModel.GetProductById(tempShoppingCart[0].product_id);
      ...
      try {
        // 更新购物车信息,把数量+1
        const result = await shoppingCartModel.UpdateShoppingCart(tempNum, user_id, product_id);
      ...
      try {
        // 新插入购物车信息
        const res = await shoppingCartModel.AddShoppingCart(user_id, product_id);
        // 判断是否插入成功
        if (res.affectedRows === 1) {
          // 如果成功,获取该商品的购物车信息
          const shoppingCart = await shoppingCartModel.FindShoppingCart(user_id, product_id);
          // 生成购物车详细信息
          const data = await methods.ShoppingCartData(shoppingCart);
  ...
  DeleteShoppingCart: async ctx => {
    ...
    if (tempShoppingCart.length > 0) {
      // 如果存在则删除
      try {
        const result = await shoppingCartModel.DeleteShoppingCart(user_id, product_id);
        // 判断是否删除成功
        if (result.affectedRows === 1) {
          ctx.body = {
            code: '001',
  ...
  UpdateShoppingCart: async ctx => {
    ...
    if (tempShoppingCart.length > 0) {
      ...
      try {
        // 修改购物车信息
        const result = await shoppingCartModel.UpdateShoppingCart(num, user_id, product_id);
        // 判断是否修改成功
        if (result.affectedRows === 1) {
          ctx.body = {
            code: '001',
