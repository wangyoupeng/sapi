...
module.exports = {
  ...
  Login: async ctx => {
    ...
    // 校验用户信息是否符合规则
    // if (!checkUserInfo(ctx, username, pwd)) {
    //   return;
    // }
  ...
  miniProgramLogin: async ctx => {
    ...
    // 连接数据库根据用户名查询用户信息
    let user = await userModel.FindUserName(openid);
    if (user.length === 0) {
      // 结果集长度为0则代表不存在该用户,先注册
      try {
        // 连接数据库插入用户信息
        let registerResult = await userModel.Register(openid, openid);
    ...
    async function login () {
      // 连接数据库根据用户名和密码查询用户信息
      let tempUser = await userModel.Login(openid, openid);
  ...
  Register: async ctx => {
    ...
    try {
      // 连接数据库插入用户信息
      let registerResult = await userModel.Register(userName, password);
      // 操作所影响的记录行数为1,则代表注册成功
      if (registerResult.affectedRows === 1) {
        ctx.body = {
          code: '001',
