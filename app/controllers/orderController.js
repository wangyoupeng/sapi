...
module.exports = {
  ...
  GetOrder: async ctx => {
    ...
    // 获取所有的订单id
    const ordersGroup = await orderModel.GetOrderGroup(user_id);
    ...
    // 获取所有的订单详细信息
    const orders = await orderModel.GetOrder(user_id);
  ...
  AddOrder: async (ctx) => {
    ...
    try {
      // 把订单信息插入数据库
      const result = await orderModel.AddOrder(products.length, data);
      ...
      if (result.affectedRows == products.length) {
        ...
        for (let i = 0; i < products.length; i++) {
          const temp = products[i];
          const res = await shoppingCartModel.DeleteShoppingCart(user_id, temp.productID);
